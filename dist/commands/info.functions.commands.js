...
function toBareJid(jid) {
  ...
  // Se vier "user:device@s.whatsapp.net", vira "user@s.whatsapp.net"
  if (d?.user) return `${d.user}@s.whatsapp.net`;
  // fallback: remove o “:device” se existir
  return String(jid).replace(/:\d+@s\.whatsapp\.net$/, '@s.whatsapp.net');
}

export async function menuCommand(client, botInfo, message, group) {
  const senderBare = toBareJid(message.sender);

  const userCtrl = new UserController();
  const userData = await userCtrl.getUser(senderBare);
  if (!userData) {
    throw new Error(infoCommands.menu.msgs.error_user_not_found);
  }

  const userType = userData.owner
    ? botTexts.user_types.owner
    : (userData.admin ? botTexts.user_types.admin : botTexts.user_types.user);

  let replyText = buildText(
    infoCommands.menu.msgs.reply,
    userData.name,
    userType,
    userData.commands
  );
  ...
  if (!message.args.length) {
    ...
    switch (commandText) {
      case '0': replyText += menu.infoMenu(botInfo); break;
      case '1': replyText += menu.stickerMenu(botInfo); break;
      case '2': replyText += menu.utilityMenu(botInfo); break;
      case '3': replyText += menu.downloadMenu(botInfo); break;
      case '4':
        replyText += message.isGroupMsg ? menu.miscGroupMenu(botInfo) : menu.miscMenu(botInfo);
        break;
      case '5':
        if (!message.isGroupMsg) throw new Error(botTexts.permission.group);
        replyText += message.isGroupAdmin ? menu.groupAdminMenu(botInfo) : menu.groupMenu(botInfo);
        break;
      default:
